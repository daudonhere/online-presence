...
**Online Presence** — Aplikasi Absensi Guru (Teacher Attendance System) for MTS AL-RIYADL school.
...
|---|---|---|
| `/` | `src/app/page.tsx` | Dashboard utama — menu absensi |
| `/scan` | `src/app/scan/page.tsx` | Scan QR untuk absensi |
...
| `/halangan/[category]` | `src/app/halangan/[category]/page.tsx` | Detail kategori halangan |
...
- **Register:** name + phone + password + retype password
- Toggle between login/register mode
- Validation: password match check on register
- **PWA Install Banner:** listens for `beforeinstallprompt`, checks `display-mode: standalone`, persists dismiss in `localStorage("pwa-install-dismissed")`, slide-up animation from bottom

### Dashboard (`/`)
- Large QR code icon in white rounded box
- "Scan QR" button → `/scan`
- "Absen Manual" button → `/manual`
- Decorative: yellow circle, blue circle, 8 yellow dots grid

### Scan QR (`/scan`)
- Animated QR placeholder (no scanning library integrated yet)
- Corner borders (yellow), pulsing scan line animation
- "Kirim Absensi" button (not functional yet)
- "Gunakan Absen Manual" link → `/manual`
...
- "Kirim Absensi" submit button
- "Status Terakhir" section showing last attendance status
...
- **Logout:** red section "Keluar dari Akun"
- **Footer:** "Absensi Guru MTS AL-RIYADL v1.0.0"

## Data Models (Proposed Prisma Schema)
...
- `POST /api/obstacles` — submit new obstacle
...
| `Header.tsx` | Client | Sticky top header: green gradient, school name badge "MTS AL-RIYADL", greeting + user name, clock + time, notification bell. Props: `greeting`, `userName`, `currentTime`, `onNotificationClick`. |
...
- Persists dismiss in `localStorage("pwa-install-dismissed")`
- Slide-up banner with install/dismiss buttons
...
| `@prisma/client` | ^7.8.0 | ORM client | Installed (no models) |
| `prisma` | ^7.8.0 | ORM CLI | Installed (no models) |
| `@tanstack/react-query` | ^5.101.2 | Server state | Unused |
| `next-auth` | ^5.0.0-beta.31 | Auth | Unused (no config) |
| `next-pwa` | ^5.6.0 | PWA support | Used |
| `next-safe-action` | ^8.5.5 | Type-safe actions | Unused |
| `zod` | ^4.4.3 | Validation | Unused |
| `lucide-react` | ^1.24.0 | Icons | Used |
| `bcryptjs` | ^3.0.3 | Password hashing | Unused |
...
online-presence/
  ...
  eslint.config.mjs
  prisma/
    schema.prisma                   # EMPTY — no models yet
  public/
    manifest.json
    ...
    icons/
      icon-192x192.png
      icon-512x512.png
    reference/                      # 10 HTML design mockups
  src/
    app/
      ...
      manual/page.tsx               # Manual attendance
      analisa/page.tsx              # Attendance analysis
      laporan/page.tsx              # Reports
      halangan/page.tsx             # Obstacles hub
      halangan/[category]/page.tsx  # Obstacle form
...
- Auth config in `src/lib/auth.ts` (does not exist yet)
- All page data is currently hardcoded/mock
- No API routes, server actions, or database models exist yet
